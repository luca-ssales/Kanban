...
(function () {
  ...
  function sanitizeState(raw) {
    ...
    if (!copy.theme || typeof copy.theme !== 'object') {
      copy.theme = { type: 'preset', presetId: 'theme-light', customColor: '#ffffff' };
    }
  ...
  async function init() {
    ...
    // Não entra direto: sempre exibe a tela de login inicial com os dados pré-preenchidos
    setAuthToken(null);
    setAuthUser(null);
    updateAuthUserUI();
  ...
  function updateAuthUserUI() {
    ...
    if (user) {
      ...
      if (emailInput) emailInput.value = 'admin@kanban.com';
      if (passInput) passInput.value = 'admin123';
    }
  }

  // Se o frontend for aberto via Live Server (ex: porta 5500), protocolo file:// ou qualquer porta != 8080,
  // roteia automaticamente as chamadas de API para o backend Node.js na porta 8080
  ...
  async function loginUser(email, senha) {
    try {
      ...
      showToast('Erro de conexão com o servidor. Verifique se o Node.js está rodando (porta 8080).', 'error');
      return false;
    }
  }
  ...
  async function logoutUser(callServer = true, reasonMsg = 'Logout realizado com sucesso!') {
    ...
    setAuthToken(null);
    setAuthUser(null);
    state = { activeBoardId: null, theme: { type: 'preset', presetId: 'theme-light' }, boards: [] };
    updateAuthUserUI();
  ...
  function setupAuthEventListeners() {
    ...
    if (logoutBtn) {
      logoutBtn.addEventListener('click', () => {
        logoutUser(true, 'Logout efetuado com sucesso!');
      });
    }
    ...
    if (sidebarLogoutBtn) {
      sidebarLogoutBtn.addEventListener('click', () => {
        logoutUser(true, 'Logout efetuado e retornado à tela de login!');
      });
    }

    const userAuthBadge = document.getElementById('userAuthBadge');
    if (userAuthBadge) {
      userAuthBadge.style.cursor = 'pointer';
      userAuthBadge.title = 'Clique para sair da conta';
      userAuthBadge.addEventListener('click', () => {
  ...
  function applyTheme(themeObj) {
    ...
    const validTheme = (themeObj && typeof themeObj === 'object') ? themeObj : { type: 'preset', presetId: 'theme-light' };
    
    if (validTheme.type === 'preset') {
      document.body.classList.add(validTheme.presetId || 'theme-light');
  ...
  function setupEventListeners() {
    ...
    if (sidebarThemeBtn) sidebarThemeBtn.addEventListener('click', () => openModal(elements.themeModal));
