...
const DATA_FILE = path.join(DATA_DIR, 'kanban_data.json');
...
const server = http.createServer((req, res) => {
  ...
  // index.html fica na raiz do projeto (não dentro de public/) conforme especificado
  const cleanUrl = req.url.split('?')[0];
  let safePath = decodeURIComponent(cleanUrl);

  if (safePath === '/' || safePath === '/index.html') {
    const rootIndexPath = path.join(__dirname, 'index.html');
    if (fs.existsSync(rootIndexPath)) {
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
      fs.createReadStream(rootIndexPath).pipe(res);
      return;
    }
  }

  // Lista de possíveis caminhos para resolver arquivos estáticos (public, assets, raiz)
  ...
  if (safePath.startsWith('/public/')) {
    candidatePaths.push(path.join(PUBLIC_DIR, safePath.replace(/^\/public\//, '')));
  }

  if (safePath.startsWith('/assets/')) {
    candidatePaths.push(path.join(PUBLIC_DIR, safePath));
    candidatePaths.push(path.join(__dirname, safePath));
  }

  if (safePath === '/favicon.ico') {
    candidatePaths.push(path.join(__dirname, 'assets', 'favcon.png'));
    candidatePaths.push(path.join(PUBLIC_DIR, 'assets', 'favcon.png'));
  }
  ...
  for (const candidate of candidatePaths) {
    try {
      ...
      if (resolved.startsWith(__dirname) && fs.existsSync(resolved) && fs.statSync(resolved).isFile()) {
        targetFile = resolved;
        break;
      }
    } catch (e) {}
